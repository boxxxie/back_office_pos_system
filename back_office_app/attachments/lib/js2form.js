...
var populateForm = 
    (function (){
	 ...
	 function formElementPopulator($node,value){
	     //sets the value of an element in a form. some elements require special treatment (checkboxs,radio)
	     
	     //these jQuery things should probably be external
	     //this is for jQuery multiselect plugin
	     
	     if(_.isNull(value) || _.isUndefined(value)){return;}

	     if($node.attr('type') == 'select' && _.isArray(value)){
		 $node.multiselect('uncheckAll');
		 $node
		     .find('option')
		     .filter(function(){return _.include(value,$(this).val());})
		     .each(function(){$(this).attr('selected',true);});
		 $node.multiselect('refresh');
		 return;
	     }
	     //this is for jQuery datepicker
	     if($node.datepicker && _.isDate(value)){
		 $node.datepicker("setDate",value);
		 console.log("datePicker for node: ");
		 console.log($node);
		 return;
	     }
